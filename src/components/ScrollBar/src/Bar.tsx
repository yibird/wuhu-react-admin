...
function Bar({ vertical, size, color = 'rgba(144, 147, 153, 0.3)', width = 6, move }: BarProps) {
  ...
  const clickTrackHandler: MouseEventHandler<HTMLDivElement> = (e) => {
    ...
    // 获取偏移量
    const offset = Math.abs(e.currentTarget.getBoundingClientRect()[direction] - e[client]);
    // 计算滚动条块的一半
    const thumbHalf = thembEl[opt.offset] / 2;
    // 获取当前组件元素的offset距离
    const currentElOffset = barRef.current![opt.offset];
    // 计算滚动条百分比
    const thumbPositionPercentage = ((offset - thumbHalf) * 100) / currentElOffset;
  ...
  const mouseMoveDocumentHandler: MouseEventHandler<HTMLDivElement> = (e) => {
    ...
    if (!wrapEl) return;
    const { direction, client, scroll, scrollSize } = opt;
    const offset = (wrapEl!.getBoundingClientRect()[direction] - e[client]) * -1;
    // 获取滚动块的点击位置
